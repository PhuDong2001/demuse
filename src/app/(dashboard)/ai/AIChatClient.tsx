...
export function AIChatClient({ user, timetableId }: AIChatClientProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col h-[calc(100dvh-7.5rem)] sm:h-[calc(100dvh-8.5rem)] space-y-3 sm:space-y-4 animate-in fade-in duration-300">
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 sm:p-5 rounded-2xl border border-[#ded7c8] bg-white shadow-xs shrink-0">
        <div className="flex items-center gap-3">
          <RobotIcon className="h-10 w-10 shrink-0" size={40} />
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-serif text-lg sm:text-2xl font-medium text-[#1c1917]">
            ...
            <p className="text-xs text-[#78716c] mt-0.5">
              {isVi
                ? "Phân tích thời khóa biểu, ca làm, tìm giờ rảnh & phát hiện trùng lịch tức thì."
                : "Ask about your commitments, find free slots, check clashes, or plan routines."}
            </p>
          </div>
        </div>
      ...
      {errorMessage && (
        <div className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-800 shrink-0">
      ...
      {/* Main Chat Box Full Height */}
      <div className="rounded-2xl border border-[#ded7c8] bg-white shadow-xs overflow-hidden flex flex-col flex-1 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 min-h-0">
          {messages.map((msg, index) => {
            ...
            return (
              ...
              >
                {isAssistant && (
                  <RobotIcon className="h-8 w-8 shrink-0" size={32} />
                )}
        ...
        <div className="px-4 py-2.5 bg-[#faf7f2]/60 border-t border-[#f0eae1] flex items-center gap-2 overflow-x-auto select-none no-scrollbar shrink-0">
        ...
        <form
          ...
          className="p-3 sm:p-4 border-t border-[#ded7c8] bg-white flex items-center gap-2 shrink-0"
        >
          <input
            type="text"
            placeholder={
